FROM bcit.io/library/alpine:3.24-latest

LABEL maintainer="chriswood@gmail.com,jesse@weisner.ca"
LABEL build_id="1790284016"
LABEL nginx_version="1.30.4"
...
RUN printf "%s%s%s\n" \
 ...
 && apk add --no-cache \
    ...
    nginx-mod-stream-geoip2=1.30.4-r1 \
    nginx-mod-http-image-filter=1.30.4-r1 \
    nginx-mod-http-xslt-filter=1.30.4-r1 \
    nginx-mod-http-js=1.30.4-r1 \
    nginx-mod-stream-js=1.30.4-r1
